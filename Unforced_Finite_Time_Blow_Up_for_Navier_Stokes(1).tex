...
The forced carrier and numerous local analytic identities are imported from the released OpenAI manuscript and its Lean repository \cite{OpenAI2026NS,OpenAIRepo2026}.  This release includes a concrete, placeholder-free \texttt{UnforcedClosure.lean} source file for the abstract closure layer, together with a pinned Lean/Mathlib manifest, exact-rational replay, source-hygiene checks, SHA-256 hashes, and reproducibility receipts.  The abstract closure source has now been kernel-compiled successfully in the pinned Lean/Mathlib environment: \texttt{lake env lean UnforcedClosure.lean} and the local \texttt{lake build} pass, while the exact-arithmetic/source-hygiene replay and Pytest suite also pass.  The PDE-specific hypotheses in Appendix~\ref{app:precise} have not yet been discharged by a compiled end-to-end theorem against the frozen upstream repository.  Accordingly, the machine-checked result presently covers the abstract closure layer, not the full unforced Navier--Stokes theorem.  Priority language remains qualified pending PDE-specific Lean integration, independent source replay, and refereeing.
...
\end{align}
The classical periodic problem asks whether every smooth divergence-free datum generates a smooth solution for all positive time.  The official Clay formulation distinguishes global smoothness with \(f\equiv0\) from breakdown alternatives allowing a smooth force \cite{FeffermanClay}.  On 8 September 2026, OpenAI released a manuscript and accompanying Lean formalization claiming finite-time breakdown with smooth forcing on both \(\R^3\) and \(\T\) \cite{OpenAI2026NS,OpenAIRepo2026}.  On 11 September 2026, the Clay Mathematics Institute stated that the problem had ``apparently been settled,'' while emphasizing that its formal evaluation and credit process would proceed under the published prize rules and was deliberately unhurried \cite{ClayAnnouncement2026}.  Accordingly, throughout this paper the released OpenAI construction and frozen repository are treated as pinned mathematical source inputs, not as an independently accepted resolution of the Millennium Prize Problem.  The question addressed here is whether the smooth forcing in that released construction can be removed while retaining the same singular carrier.
...
\end{align}
The inverse construction below is carried out in a pressure-treated source space: equivalently, one may regard the linear operator as acting on the solenoidal velocity correction together with its zero-mean pressure component, or eliminate that pressure component and work with the corresponding solenoidal reduction.  All identities of the form \(LG=I\) are identities in this pressure-treated source space.  The support statements in the core branch refer to the velocity component and are not inferred from a nonlocal global Leray projection.  Section~\ref{sec:periodic} reconstructs the physical pressure explicitly and thereby returns to the unprojected equation \eqref{eq:expanded}.
...
\end{equation}
For the fixed-point argument these spaces are taken with their Banach completions.  In particular, each finite-jet space \(E_m\) is used as a Banach space, the weighted harmonic norms \eqref{eq:Xnorm}--\eqref{eq:Ynorm} are weighted \(\ell^1\)-sums of Banach norms, and the core/regular direct sums use the corresponding product norm.  Hence \(X_{s,m,\rho}\) and \(Y_{s,m,\rho}\) are complete, and the closed ball used below is a complete metric space.  The finite order \(m\) is chosen above the Sobolev/product threshold needed for a classical strong solution; smoothness on compact subslabs is then recovered by the parabolic bootstrap described after \eqref{eq:reszero}.
...
\label{eq:block2}
\end{equation}
There is one further typing condition that is logically essential: the regular-to-core mismatch must actually lie in the core source class on which \(G_c\) is a right inverse,
\begin{equation}
\boxed{
K_{\rm full}G_{\rm reg}S_r(Y)\subseteq Y_c,
\qquad
LG_c y=y\quad(y\in Y_c).}
\label{eq:corerange}
\end{equation}
Equivalently,
\begin{equation}
LG_cK_{\rm full}G_{\rm reg}S_r
=K_{\rm full}G_{\rm reg}S_r.
\label{eq:corecancel}
\end{equation}
Define
\begin{equation}
...
\label{eq:Gglobal}
\end{equation}
Then \eqref{eq:block1}, \eqref{eq:block2}, and \eqref{eq:corecancel} give
\begin{align}
LG
...
\label{eq:LGI}
\end{align}
This identity is exact; no endpoint-flat remainder remains.  Notice that the norm estimate \eqref{eq:Knative} controls the \emph{size} of the mismatch but does not by itself imply the range inclusion \eqref{eq:corerange}; that inclusion is therefore stated explicitly in Appendix~\ref{app:precise} and is part of the PDE-specific verification gate.
...
\label{eq:Phi}
\end{equation}
Let \(M_G=\norm G\) and use \(C_B\) from \eqref{eq:weightedB}.  Because these constants may themselves depend on the late slab, the required small quantity is the complete contraction parameter
\begin{equation}
\Gamma(q_*,T):=4M_G(q_*,T)C_B(q_*,T)\eta(q_*,T).
\label{eq:Gamma}
\end{equation}
By the quantifier order \eqref{eq:quantifier} and Hypothesis~\ref{hyp:eta}, choose \(q_*\) first and then \(T\) so that \(\Gamma(q_*,T)<1\).  On the ball
\begin{equation}
\cB_{2\eta}:=\{v:\norm v_X\le2\eta\},
\label{eq:ball}
\end{equation}
...
\begin{equation}
\boxed{\Gamma=4M_GC_B\eta<1}
\label{eq:smallcondition}
\end{equation}
implies that \(\Phi\) preserves \(\cB_{2\eta}\) and is a contraction.  Banach's theorem applies because the graph space was taken complete in Section~\ref{sec:spaces}, and gives a unique fixed point
\begin{equation}
v=-G(F_*+B(v,v)).
\label{eq:fixedpoint}
\end{equation}
...
\label{eq:reszero}
\end{equation}
After pressure reconstruction this is precisely \eqref{eq:exactgoal}.  Choose \(m\) above the classical product/embedding threshold.  On every compact subslab \([t_0,1-\delta]\), \(\delta>0\), the carrier coefficients and forcing are smooth and nonsingular; the fixed point is therefore a strong solution there.  Standard periodic parabolic regularity then bootstraps \(v\), and the elliptic pressure equation bootstraps \(\pi\), through every finite derivative order.  Hence \((v,\pi)\) is \(C^\infty\) for every \(t<1\).  No uniform smooth bound at the singular endpoint is asserted or needed.
...
\label{eq:divcorr}
\end{equation}
Taking the divergence of the physical correction equation \eqref{eq:expanded}, using \(\diver U_*=\diver v=0\), gives the zero-mean periodic Poisson equation
\begin{equation}
\boxed{
-\Delta\pi
=\nabla\!\cdot F_*
...
\label{eq:pressure}
\end{equation}
Both terms on the right have zero spatial mean: the first is the divergence of a periodic vector field and the second is a periodic double divergence.  Standard torus elliptic theory therefore gives a unique zero-mean smooth \(\pi\) whenever the velocity fields and \(F_*\) are smooth.  In a Leray-reduced formulation, \eqref{eq:pressure} restores exactly the gradient component discarded by the projection; in a coupled velocity-pressure formulation it is the corresponding compatibility identity.  In the whole-space analog, second-order Riesz transforms give the same pressure structure; the bounded Fourier multiplier is formalized in \path{R3PressureFourier.lean} and classically belongs to Calder\'on--Zygmund theory \cite{Stein1970,OpenAIRepo2026}.
...
The released OpenAI paper \cite{OpenAI2026NS} supplies the carrier geometry, oscillatory stress realization, compact forcing, and exact periodic periodization used here as source inputs.  Its entire published reference list has been retained in the bibliography of the present paper: \cite{AlbrittonBrueColombo2022,BillantGallaire2005,BillantGallaire2013,BuckmasterVicol2019,CKN1982,CordobaMartinezZoroa2023,CordobaMartinezZoroa2024,CordobaMartinezZoroaZheng2026,CraikCriminale1986,DaneriSzekelyhidi2017,EscauriazaSereginSverak2003,Euler1757,FeffermanClay,FriedlanderVishik1991,LeibovichStewartson1983,Leray1934,LifschitzHameiri1991,Navier1827,SinghSridhar2017,Stein1970,Stokes1845,Tao2016}.  The present argument does not depend on Clay acceptance of the broader Millennium claim; it depends only on the explicitly identified carrier, scaling, support, gluing, and residual identities in the released manuscript and frozen Lean repository, whose applicability to the unforced closure is separately audited here.  The new claim here is not a reconstruction of those 166 pages.  It is an exact late-slab removal of the smooth force by a new inverse-and-fixed-point layer.
...
\item Smallness of \(\eta(q_*,T)\) alone is not enough if \(M_G\) or \(C_B\) varies with the slab.  The actual release parameter is \(\Gamma=4M_GC_B\eta\), and the sequential quantifier order \eqref{eq:quantifier} is used to make \(\Gamma<1\).
\item A norm bound on \(K_{\rm full}G_{\rm reg}S_r\) does not imply that this mismatch lies in the core source class.  The explicit range condition \eqref{eq:corerange} is therefore required for the exact cancellation \(LG=I\).
\item Pressure reconstruction cannot omit the divergence of the original force unless that force has first been proved solenoidal.  Equation~\eqref{eq:pressure} retains \(\nabla\!\cdot F_*\) explicitly.
\end{enumerate}
...
\]
cancels the full regular-to-core mismatch and yields \(LG=I\) exactly, provided the mismatch satisfies the explicit core-range condition \eqref{eq:corerange}.  Separating this typing/support condition from the quantitative coupling estimate removes a hidden implication that a small norm alone would not justify.
...
\]
is closed under the source-backed estimates enumerated in Appendix~\ref{app:precise}.  The released OpenAI forced theorem is accompanied by a Lean formalization \cite{OpenAIRepo2026}; its institutional evaluation remains separate from the source-level use made here.  For the new layer, the distributed formal package contains \texttt{UnforcedClosure.lean}, a pinned Lean/Mathlib environment, exact-rational tests, source-hygiene checks excluding \texttt{sorry}, \texttt{admit}, new \texttt{axiom} declarations, \texttt{unsafe}, and \texttt{native\_decide}, plus SHA-256 hashes and test receipts.  The abstract closure source has now been kernel-compiled successfully in the pinned environment, and the local Lake build together with the exact-arithmetic and Pytest replays pass.  The fixed-point residual theorem therefore has a checked Lean proof rather than merely supplied source.  The remaining formal burden is the PDE-specific one: Hypotheses~A.1--A.7, including the explicit core-range condition and complete contraction-smallness gate, must be discharged by concrete theorem applications against the frozen upstream repository, and the resulting end-to-end dependency closure must be audited.  Independent source replay, PDE-specific integration, and expert refereeing are therefore still required.

This distinction is especially important for priority.  The public OpenAI result is explicitly forced \cite{OpenAI2026NS,OpenAIRepo2026}, and Clay's evaluation and credit process remains ongoing \cite{ClayAnnouncement2026}.  If the imported carrier and the present PDE-specific unforced closure both survive independent verification, then, to our knowledge, the construction supplies a finite-time blow-up solution from smooth data for the full unforced three-dimensional incompressible Navier--Stokes equations on the periodic torus.  Any priority claim attached to that statement remains provisional until both the imported source result and the new unforcing bridge have been externally checked.

\subsection{Conclusion}
The central result is an exact nonlinear unforcing mechanism for a periodic Navier--Stokes blow-up carrier under the explicitly enumerated release hypotheses.  Its essential ingredients are a quantitatively small core parametrix defect, a Neumann right inverse, an exact core--regular cancellation with an explicit range condition, a graph norm that correctly accounts for the derivative in convection, a complete contraction-smallness gate, and a weighted fixed point that remains subleading to the singular carrier.  The physical pressure reconstruction retains the divergence of the original smooth force and therefore closes the full unprojected equation.  The final field solves the unforced equation on a late periodic slab and retains the carrier's nonzero singular coefficient.  The proof package is designed so that a referee can inspect each logical dependency without relying on informal computational evidence: numerical scripts are relegated to reproducibility checks, while the mathematical argument is stated in theorem--lemma form with explicit assumptions and estimates.
...
\begin{hypothesis}[Regular inverse and core-range coupling]\label{hyp:reg}
For fixed \(q_*\) and sufficiently short late slab there is a bounded periodic regular inverse \(G_{\rm reg}\) for the smooth regular Oseen problem in the same pressure-treated source space used by the core construction, with all coefficient, transition, partition-commutator, and pressure/Hodge effects included in \(K_{\rm full}\).  In addition, the mismatch obeys the typing/support condition
\[
K_{\rm full}G_{\rm reg}S_r(Y)\subseteq Y_c,
\]
so that \(LG_cK_{\rm full}G_{\rm reg}S_r=K_{\rm full}G_{\rm reg}S_r\).  Support localization here concerns the velocity component; no support claim is inferred from a nonlocal Leray projection.
\end{hypothesis}

\begin{hypothesis}[Contraction smallness]\label{hyp:eta}
For the exact global right inverse \(G\) in \eqref{eq:Gglobal}, set \(\eta=\norm{GF_*}_X\), \(M_G=\norm G\), and let \(C_B\) be the bilinear constant in \eqref{eq:weightedB}.  After first fixing an admissible \(q_*\), the late slab can be chosen so that
\[
\Gamma(q_*,T)=4M_G(q_*,T)C_B(q_*,T)\eta(q_*,T)<1.
\]
In particular, the hypothesis controls the complete contraction parameter and does not infer contraction from \(\eta\to0\) alone when \(M_G\) or \(C_B\) may depend on \(T\).
\end{hypothesis}
...
\end{theorem}
\begin{proof}
Hypotheses~\ref{hyp:param}--\ref{hyp:band} give \(G_c\) by the Neumann argument.  Hypothesis~\ref{hyp:reg}, including the core-range condition \eqref{eq:corerange}, and the algebra in \eqref{eq:block1}--\eqref{eq:LGI} give \(LG=I\).  Proposition~\ref{prop:Bappendix} gives the quadratic map.  Hypothesis~\ref{hyp:eta} is exactly \eqref{eq:smallcondition}; completeness of \(X\) therefore permits Banach's theorem and gives \eqref{eq:fixedpoint}.  Applying \(L\) gives the pressure-treated residual identity \eqref{eq:reszero}.  The periodic Poisson problem \eqref{eq:pressure}, now including \(\nabla\!\cdot F_*\), reconstructs the physical pressure correction and yields the unprojected equation.  Standard parabolic regularity on every compact subslab upgrades the strong fixed point to \(C^\infty\) for \(t<1\).  Finally, the positive margins in \eqref{eq:margins} and \eqref{eq:Kmargin} give \eqref{eq:allsub}, and \eqref{eq:carrierblow} gives \eqref{eq:finalblow}.
\end{proof}
...
Starting from \eqref{eq:Gglobal}, the explicit range hypothesis \eqref{eq:corerange} gives \eqref{eq:corecancel}; therefore
\begin{align}
LG
...
\end{align}
This makes the logic explicit: \eqref{eq:Knative}--\eqref{eq:Kmargin} controls the magnitude of the coupling, while \eqref{eq:corerange} supplies the separate range/support statement needed to apply \(G_c\).  Neither assertion is substituted for the other.
...
The fixed point identity \eqref{eq:fixedpoint} and \(LG=I\) give exact cancellation in the pressure-treated source space.  In a coupled velocity-pressure formulation this is already the physical correction equation.  In a solenoidal reduction, let
\[
R:=L_*v+F_*+B(v,v).
\]
The fixed-point equation gives vanishing solenoidal component of \(R\).  The remaining component is therefore a periodic gradient.  Define the zero-mean scalar \(\pi\) by \eqref{eq:pressure}; since
\[
-\Delta\pi=\nabla\!\cdot R,
\]
we have \(R+\nabla\pi=0\).  Consequently
\begin{align}
\N_\nu(U_*+v,P_*+\pi)
&=F_*+L_*v+B(v,v)+\nabla\pi\notag\\
&=0.
...
\end{align}
Thus the pressure reconstruction closes the full unprojected Navier--Stokes residual, including the possible non-solenoidal part of the original smooth force.
...
The abstract closure layer is now kernel-checked in the pinned Lean/Mathlib environment.  CI successfully compiles \texttt{UnforcedClosure.lean} and the local Lake target; the exact-rational/source-hygiene replay and Pytest suite pass.  The source-hygiene gate rejects \texttt{sorry}, \texttt{admit}, new \texttt{axiom} declarations, \texttt{unsafe}, and \texttt{native\_decide}.  Lean's dependency printout reports no \texttt{sorryAx} for the fixed-point theorem, and the finite Neumann support theorem is axiom-free.  This is a formal certificate for the abstract closure logic, not yet a formal proof of the complete PDE theorem.
...
\midrule
Upstream forced theorem & source-backed Lean result & Imported from the frozen OpenAI repository; not reproved or institutionally certified here. \\
Abstract scalar/exponent layer & kernel compiled & Exact identities and positivity margins compile in the pinned environment. \\
Fixed-point residual algebra & kernel compiled & Corrected theorem proof compiles; no \texttt{sorryAx} is reported. \\
Finite Neumann support lemma & kernel compiled & Finite support-preservation theorem compiles and is axiom-free. \\
PDE-specific unforced wrappers & \statushold & Hypotheses~A.1--A.7 are not yet discharged by concrete imported Lean theorems. \\
Full unforced theorem in Lean & \statushold & No end-to-end machine certification is claimed until the PDE bridge compiles and is audited. \\
\bottomrule
\end{tabularx}
\caption{Formal-certificate status for this release.  ``Kernel compiled'' is intentionally distinguished from PDE-level discharge.}
\label{tab:leanstatus}
\end{table}

\paragraph{Decisive formal release gate.}
The abstract kernel-compilation gate is now satisfied.  A future version may call the full unforced theorem machine-checked only after the remaining PDE gates are closed: (a) the frozen upstream repository is built and imported in the same pinned environment; (b) the analytic hypotheses in Appendix~\ref{app:precise} are replaced one by one by concrete theorem applications; (c) the resulting PDE-specific unforced theorem compiles; and (d) its dependency closure is audited for unintended axioms or placeholder proofs.  Until then, the Lean package machine-checks the abstract closure layer while the end-to-end Navier--Stokes formalization remains a separate verification target.
...
The companion \texttt{pytest} file uses deterministic exact arithmetic; every test is finite and non-blocking.  In the current release the exact-arithmetic and source-hygiene suite reports \texttt{3 passed}, and the pinned CI environment additionally kernel-compiles \texttt{UnforcedClosure.lean} and builds the local Lake target successfully.  These checks certify the abstract closure implementation; no network, database, stochastic simulation, or empirical fluid computation enters the mathematical proof.
...
\item Verify that \(K_{\rm full}\) includes every difference between the full and regular operators, including cutoff commutators and pressure/Hodge terms, and separately verify the core-range condition \eqref{eq:corerange}; a small coupling norm is not a substitute for that typing statement.
\item Verify the support-preservation hypothesis for \(P_c\), and hence through the Neumann series.
\item Check the sequential quantifiers \(q_*\) then \(T(q_*)\), and verify the complete contraction parameter \(\Gamma=4M_GC_B\eta<1\) rather than only \(\eta\to0\).
\item Check periodic pressure reconstruction and zero-mean compatibility, including the \(\nabla\!\cdot F_*\) term in \eqref{eq:pressure} unless a solenoidal-force reduction is proved separately.
...
\item Replay the exact-rational and source-hygiene tests and verify the pinned CI kernel compilation of \texttt{UnforcedClosure.lean}.  For a machine-checked unforced release, next build and import the frozen upstream repository, replace the hypotheses in Appendix~\ref{app:precise} one by one by concrete theorem applications, compile the resulting PDE-specific unforced theorem, and audit its dependency closure for placeholder proofs or unintended axioms.
\end{enumerate}
